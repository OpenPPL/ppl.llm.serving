...
RetCode TopPTopKSampler::Process(const float* logits_device, const float* temperatures_host, int32_t batch,
                                 int32_t* output_host) {
    ...
    if (top_p_ != 0.0) {
        pad_vocab_size = ppl::kernel::llm::cuda::pmx::flash_sample_top_p_get_pad_vocab_size(vocab_size_);
    ...
    RetCode rc;
    if (top_p_ == 0.0) {
        rc = ppl::kernel::llm::cuda::pmx::sample_argmax(stream_, logits_device, batch, vocab_size_, cu_output_);
    } else {
        ...
        rc = ppl::kernel::llm::cuda::pmx::flash_sample_top_p(stream_, logits_device, batch, vocab_size_,
                                                             temperatures_device, top_p_, sorted_value, sorted_index,
